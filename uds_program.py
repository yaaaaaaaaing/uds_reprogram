...
class uds_request:
    ...
    def uds_session_request(self,session_type,app_flag):
        req_data = [0x10,session_type]
        ret_value,resp_data = self.cantp_sender.uds_cantp_send(req_data,app_flag)
        if ret_value != True:
            self.cantp_sender.bus_shutdown()
            raise ValueError("UDS Session Request Failed!")
    def uds_ecu_reset_request(self,reset_type,app_flag):
        req_data = [0x11,reset_type]
        ret_value,resp_data = self.cantp_sender.uds_cantp_send(req_data,app_flag)
        if ret_value != True:
            self.cantp_sender.bus_shutdown()
            raise ValueError("UDS ECU Reset Request Failed!")
    def uds_write_data_request(self,record_number,data_bytes,app_flag):
    ...
    def uds_security_access_request(self,level,app_flag):
        req_data = [0x27,level]
        ret_value,resp_data = self.cantp_sender.uds_cantp_send(req_data,app_flag)
        if ret_value != True:
            self.cantp_sender.bus_shutdown()
    ...
    def uds_security_access_key_send(self,level,app_flag):
    ...
    def uds_routine_control_request(self,rc_type,routine_id,data_list,app_flag):
    ...
    def uds_program_start_request(self,address,length):
        ...
        ret_value,resp_data = self.cantp_sender.uds_cantp_send(req_data,False)
        if ret_value != True:
            self.cantp_sender.bus_shutdown()
