...
tk.Label(root, text="Request Frame ID(HEX):").grid(row=0, column=0, padx=10, pady=5, sticky="w")
entry_req_id = tk.Entry(root, width=5)
entry_req_id.grid(row=0, column=1, padx=10)

tk.Label(root, text="Response Frame ID(HEX):").grid(row=1, column=0, padx=10, pady=5, sticky="w")
entry_resp_id = tk.Entry(root, width=5)
entry_resp_id.grid(row=1, column=1, padx=10)

tk.Label(root, text="CAN 类型：").grid(row=2, column=0, padx=10, pady=5, sticky="w")
can_options = ["canfd", "classic"]
can_type_var = tk.StringVar(value=can_options[0])
tk.OptionMenu(root, can_type_var, *can_options).grid(row=2, column=1, padx=10, pady=5, sticky="w")

tk.Label(root, text="报文长度：").grid(row=3, column=0, padx=10, pady=5, sticky="w")
frame_len_options = [8,12,16,20,24,32,48,64]
frame_len_var = tk.StringVar(value=frame_len_options[0])
tk.OptionMenu(root, frame_len_var, *frame_len_options).grid(row=3, column=1, padx=10, pady=5, sticky="w")

tk.Label(root, text="选择seed/key dll文件:").grid(row=4, column=0, padx=10, pady=5, sticky="w")
dll_path = tk.StringVar()
tk.Button(root, text="浏览", command=choose_dll_file).grid(row=4, column=1, sticky="w", padx=10)
dll_display = tk.Label(root, text="未选择文件", fg="gray")
dll_display.grid(row=4, column=2, sticky="w", padx=10)

tk.Label(root, text="选择seed/key flashdrv_hex文件:").grid(row=5, column=0, padx=10, pady=5, sticky="w")
flashdrv_hex_path = tk.StringVar()
tk.Button(root, text="浏览", command=choose_flashdrv_hex_file).grid(row=5, column=1, sticky="w", padx=10)
flashdrv_hex_display = tk.Label(root, text="未选择文件", fg="gray")
flashdrv_hex_display.grid(row=5, column=2, sticky="w", padx=10)

tk.Label(root, text="选择seed/key flashdrv_sig文件:").grid(row=6, column=0, padx=10, pady=5, sticky="w")
flashdrv_sig_path = tk.StringVar()
tk.Button(root, text="浏览", command=choose_flashdrv_sig_file).grid(row=6, column=1, sticky="w", padx=10)
flashdrv_sig_display = tk.Label(root, text="未选择文件", fg="gray")
flashdrv_sig_display.grid(row=6, column=2, sticky="w", padx=10)

tk.Label(root, text="选择seed/key target_hex文件:").grid(row=7, column=0, padx=10, pady=5, sticky="w")
target_hex_path = tk.StringVar()
tk.Button(root, text="浏览", command=choose_target_hex_file).grid(row=7, column=1, sticky="w", padx=10)
target_hex_display = tk.Label(root, text="未选择文件", fg="gray")
target_hex_display.grid(row=7, column=2, sticky="w", padx=10)

tk.Label(root, text="选择seed/key target_sig文件:").grid(row=8, column=0, padx=10, pady=5, sticky="w")
target_sig_path = tk.StringVar()
tk.Button(root, text="浏览", command=choose_target_sig_file).grid(row=8, column=1, sticky="w", padx=10)
